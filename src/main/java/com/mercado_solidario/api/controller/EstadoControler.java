...
public class EstadoControler {
	...
	@GetMapping("/{estadoId}") // -> /estados/estadoId 
	...
	//  -> /estados/por-nome?nome = nome_buscado
	@GetMapping("/por-nome")
	public List<Estado> EstadosPorNome(String nome) { 
		return estadoRepository.findAllByNomeContains(nome);
	}
	
//  -> /estados/sigla?sigla = sigla_buscada
	@GetMapping("/sigla")
	public List<Estado> EstadosPorsSigla(String sigla) { 
		return estadoRepository.findAllBySiglaLike(sigla);
	}
